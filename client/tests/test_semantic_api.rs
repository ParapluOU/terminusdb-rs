...
async fn setup_test_client() -> anyhow::Result<(TerminusDBHttpClient, BranchSpec)> {
    ...
    client.insert_entity_schema::<SemanticTestModel>(args.clone()).await.ok();
...
async fn test_create_instance_generates_commit() -> anyhow::Result<()> {
    ...
    assert!(result2.is_err(), "create_instance should fail if instance already exists");
...
async fn test_update_instance_requires_existing() -> anyhow::Result<()> {
    ...
    assert!(result.is_err(), "update_instance should fail if instance doesn't exist");
    ...
    assert!(update_result.commit_id.is_some(), "update_instance should generate a commit with ID");
...
async fn test_save_instance_creates_or_updates() -> anyhow::Result<()> {
    ...
    assert!(result1.commit_id.is_some(), "save_instance should generate a commit with ID");
    
    // Verify it exists
    let mut deserializer = terminusdb_client::deserialize::DefaultTDBDeserializer;
    let retrieved = client.get_instance::<SemanticTestModel>("semantic_test_save_001", &spec, &mut deserializer).await?;
    ...
    assert!(result2.commit_id.is_some(), "save_instance update should generate a commit with ID");
    
    // Verify update worked
    let retrieved2 = client.get_instance::<SemanticTestModel>("semantic_test_save_001", &spec, &mut deserializer).await?;
...
async fn test_semantic_methods_generate_proper_version_history() -> anyhow::Result<()> {
    ...
    let commit1 = create_result.extract_commit_id().expect("Should have commit ID");
    ...
    let commit2 = update_result.extract_commit_id().expect("Should have commit ID");
    ...
    let commit3 = save_result.extract_commit_id().expect("Should have commit ID");
    println!("Saved instance in commit: {}", commit3);
    
    // Verify all commits are different
    assert_ne!(commit1, commit2, "Each operation should generate a unique commit");
    assert_ne!(commit2, commit3, "Each operation should generate a unique commit");
    assert_ne!(commit1, commit3, "Each operation should generate a unique commit");
    
    // Get version history
    let history = client.get_instance_history::<SemanticTestModel>(fixed_id, &spec, None).await?;
    println!("Found {} commits in history", history.len());
    
    // Should have at least 3 commits
    assert!(history.len() >= 3, "Should have at least 3 commits in history");
    ...
    for (i, (model, commit)) in versions.iter().enumerate() {
        println!("Version {}: {} (value={}) in commit {}", i+1, model.name, model.value, commit);
    }
...
async fn test_replace_instance_is_alias_for_update() -> anyhow::Result<()> {
    ...
    assert!(replace_result.commit_id.is_some(), "replace_instance should generate a commit with ID");
    
    // Verify the update worked
    let mut deserializer = terminusdb_client::deserialize::DefaultTDBDeserializer;
    let retrieved = client.get_instance::<SemanticTestModel>("semantic_test_replace_001", &spec, &mut deserializer).await?;
