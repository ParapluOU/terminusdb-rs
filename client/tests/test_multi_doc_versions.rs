...
mod tests {
    ...
    async fn test_get_multiple_instance_versions() -> anyhow::Result<()> {
        ...
        server
            .with_tmp_db("test_multi_versions", |client, spec| async move {
                ...
                let mut doc1_commits: Vec<CommitId> = Vec::new();
                for i in 1..=3 {
                ...
                let mut doc2_commits: Vec<CommitId> = Vec::new();
                for i in 1..=2 {
                    let doc = MultiDocTest {
                ...
                let versions = client
                    .get_multiple_instance_versions::<MultiDocTest>(
                        queries,
                        &spec,
                        &mut deserializer,
                    )
                ...
                let doc1_version_numbers: Vec<i32> =
                    doc1_versions.iter().map(|(doc, _)| doc.version).collect();
                ...
                println!("\n=== Test 2: Empty query list ===");
                let empty_queries: Vec<(&str, Vec<CommitId>)> = vec![];
                let versions = client
                    .get_multiple_instance_versions::<MultiDocTest>(
                        empty_queries,
                        &spec,
    ...
    async fn test_list_multiple_instance_versions() -> anyhow::Result<()> {
        ...
        server
            .with_tmp_db("test_list_multi_versions", |client, spec| async move {
                setup_schema(&client, &spec).await?;
                ...
                let all_versions = client
                    .list_multiple_instance_versions::<MultiDocTest>(
                        instance_ids,
                        &spec,
                        &mut deserializer,
                    )
    ...
    async fn test_multi_doc_overlapping_commits() -> anyhow::Result<()> {
        ...
        server
            .with_tmp_db("test_overlapping_commits", |client, spec| async move {
                setup_schema(&client, &spec).await?;
