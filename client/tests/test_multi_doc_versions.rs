...
async fn test_get_multiple_instance_versions() -> anyhow::Result<()> {
    ...
    let doc1_id = &format!("doc1_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    let doc2_id = &format!("doc2_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    let doc3_id = &format!("doc3_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    ...
    let queries = vec![
        (doc1_id.as_str(), vec![doc1_commits[0].clone(), doc1_commits[2].clone()]), // v1 and v3
    ...
    let versions = client.get_multiple_instance_versions::<MultiDocTest>(
        queries,
        &spec,
        &mut deserializer
    ).await?;
    ...
    assert_eq!(versions.len(), 3, "Should have versions for all 3 documents");
    
    // Verify doc1 has 2 versions (v1 and v3)
    let doc1_versions = versions.get(doc1_id).unwrap();
    assert_eq!(doc1_versions.len(), 2);
    let doc1_version_numbers: Vec<i32> = doc1_versions.iter()
    ...
    for (doc_id, doc_versions) in &versions {
        ...
        for (doc, commit_id) in doc_versions {
            println!("  - {} (v{}) in commit {}", doc.content, doc.version, commit_id);
        }
    }
    ...
    assert_eq!(versions.len(), 0, "Empty queries should return empty HashMap");
    ...
    assert_eq!(versions.len(), 2, "Should only have versions for existing documents");
...
async fn test_list_multiple_instance_versions() -> anyhow::Result<()> {
    ...
    let doc1_id = &format!("doc1_list_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    let doc2_id = &format!("doc2_list_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    ...
    let mut deserializer = terminusdb_client::deserialize::DefaultTDBDeserializer;
    let all_versions = client.list_multiple_instance_versions::<MultiDocTest>(
        instance_ids,
        &spec,
        &mut deserializer
    ).await?;
    
    println!("Retrieved all versions for {} documents", all_versions.len());
    
    // Verify results
    assert_eq!(all_versions.len(), 2, "Should have versions for 2 existing documents");
    ...
    for doc_id in &[doc1_id.as_str(), doc2_id.as_str()] {
        let versions = client.list_instance_versions::<MultiDocTest>(
            doc_id,
            &spec,
            &mut deserializer
        ).await?;
    ...
    for (doc_id, multi_versions) in all_versions {
        ...
        assert_eq!(multi_versions.len(), seq_versions.len(), 
                   "Version count should match for {}", doc_id);
    }
...
async fn test_multi_doc_overlapping_commits() -> anyhow::Result<()> {
    ...
    let doc1_id = &format!("overlap1_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    let doc2_id = &format!("overlap2_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    ...
    let queries = vec![
        (doc1_id.as_str(), vec![commit1.clone(), commit2.clone(), commit3.clone()]),
        (doc2_id.as_str(), vec![commit1.clone(), commit2.clone(), commit3.clone()]),
    ];
    ...
    let doc1_versions = versions.get(doc1_id).unwrap();
    assert!(doc1_versions.len() >= 2, "Doc1 should have at least 2 versions");
    
    // Doc2 should have version from commit2 only
    let doc2_versions = versions.get(doc2_id).unwrap();
    assert!(doc2_versions.len() >= 1, "Doc2 should have at least 1 version");
