...
async fn setup_test_client() -> anyhow::Result<(TerminusDBHttpClient, BranchSpec)> {
    ...
    client.insert_entity_schema::<VersionTestModel>(args).await.ok();
...
async fn test_semantic_api_creates_queryable_versions() -> anyhow::Result<()> {
    ...
    let fixed_id = &format!("semantic_version_test_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    ...
    for (i, commit_id) in commit_ids.iter().enumerate() {
        ...
        let query = WoqlBuilder::new()
            .triple(vars!("Subject"), "rdf:type", node("@schema:VersionTestModel"))
            ...
            .select(vec![vars!("Subject"), vars!("Name"), vars!("Version"), vars!("Data")])
            .using(&collection)
            .finalize();
            ...
            Ok(result) => {
                ...
                for binding in &result.bindings {
                    if let (Some(name), Some(version), Some(data)) = 
                        (binding.get("Name"), binding.get("Version"), binding.get("Data")) {
                        println!("  - Name: {:?}, Version: {:?}, Data: {:?}", name, version, data);
    ...
    if found_versions.is_empty() {
        ...
        println!("✅ Found {} version(s) via WOQL queries", found_versions.len());
        for (commit_num, name, version) in &found_versions {
            println!("  Commit {}: Name={:?}, Version={:?}", commit_num, name, version);
        }
    }
...
async fn test_direct_rest_api_time_travel() -> anyhow::Result<()> {
    ...
    let fixed_id = &format!("rest_api_test_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0));
    ...
    match client.get_instance::<VersionTestModel>(fixed_id, &commit_spec, 
                                                  &mut terminusdb_client::deserialize::DefaultTDBDeserializer).await {
        Ok(instance) => {
            println!("✅ Successfully retrieved instance from commit {}", commit_id);
