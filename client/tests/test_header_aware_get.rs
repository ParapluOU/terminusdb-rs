...
async fn test_get_instance_with_headers() -> anyhow::Result<()> {
    ...
    let result = client.get_instance_with_headers::<TestProduct>(
        "laptop001",
        &spec,
        &mut deserializer
    ).await?;
...
async fn test_get_instances_with_headers() -> anyhow::Result<()> {
    ...
    let result = client.get_instances_with_headers::<TestProduct>(
        ids,
        &spec,
        opts,
        &mut deserializer
    ...
    let mouse = retrieved_products.iter().find(|p| p.name == "Wireless Mouse").unwrap();
    assert_eq!(mouse.price, 29.99);
    
    let keyboard = retrieved_products.iter().find(|p| p.name == "Mechanical Keyboard").unwrap();
    ...
    assert!(commit_id.is_some(), "Expected commit ID in response headers");
    let commit_id = commit_id.unwrap();
    println!("Retrieved {} products from commit: {}", retrieved_products.len(), commit_id);
...
async fn test_get_all_instances_with_headers() -> anyhow::Result<()> {
    ...
    assert!(all_products.len() >= 2, "Expected at least 2 products, got {}", all_products.len());
    
    // Verify we got a commit ID
    let commit_id = result.extract_commit_id();
    assert!(commit_id.is_some(), "Expected commit ID in response headers");
    let commit_id = commit_id.unwrap();
    println!("Retrieved {} total products from commit: {}", all_products.len(), commit_id);
...
async fn test_simplified_get_latest_version() -> anyhow::Result<()> {
    ...
    let latest_commit = client.get_latest_version::<TestProduct>("tablet001", &spec).await?;
    
    assert!(latest_commit.starts_with("ValidCommit/"), "Expected ValidCommit/ prefix, got: {}", latest_commit);
    ...
    let new_latest_commit = client.get_latest_version::<TestProduct>("tablet001", &spec).await?;
    
    // Verify it's a different commit
    assert_ne!(latest_commit, new_latest_commit, "Expected different commit ID after update");
...
async fn test_commit_id_consistency() -> anyhow::Result<()> {
    ...
    let args = DocumentInsertArgs::from(spec.clone());
    let (result, insert_commit_id) = client.insert_instance_with_commit_id(&product, args).await?;
    ...
    let get_commit_id = result.extract_commit_id().expect("Expected commit ID from GET");
    ...
    assert_eq!(insert_commit_id, get_commit_id, 
