...
async fn test_schemas_macro() -> anyhow::Result<()> {
    ...
    client.insert_documents(schema_vec.iter().collect(), args).await?;
    ...
    client.create_instance(&person, instance_args.clone()).await?;
    client.create_instance(&company, instance_args.clone()).await?;
    client.create_instance(&product, instance_args.clone()).await?;
...
async fn test_insert_schemas_tuple() -> anyhow::Result<()> {
    ...
    // Test with 3-tuple  
    client.insert_schemas::<(Person, Company, Product)>(args.clone()).await?;
...
async fn test_schemas_macro_empty() -> anyhow::Result<()> {
    ...
    assert_eq!(empty_schemas.len(), 0, "Empty macro should return empty vec");
...
async fn test_mixed_approaches_comparison() -> anyhow::Result<()> {
    ...
    // Test macro approach
    client.insert_documents(macro_schemas.iter().collect(), args.clone().as_schema()).await?;
