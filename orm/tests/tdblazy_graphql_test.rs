...
#[tokio::test]
async fn test_tdblazy_schema_creates_link() -> anyhow::Result<()> {
    let server = TerminusDBServer::test_instance().await?;
    let client = server.client().await?;

    let db_name = "tdblazy_schema_test";
...
#[tokio::test]
async fn test_tdblazy_forward_relation() -> anyhow::Result<()> {
    let server = TerminusDBServer::test_instance().await?;
    let client = server.client().await?;

    let db_name = "tdblazy_forward_test";
    let _ = client.delete_database(db_name).await;
    client.ensure_database(db_name).await?;
...
#[tokio::test]
async fn test_tdblazy_reverse_relation() -> anyhow::Result<()> {
    let server = TerminusDBServer::test_instance().await?;
    let client = server.client().await?;

    let db_name = "tdblazy_reverse_test";
    let _ = client.delete_database(db_name).await;
    client.ensure_database(db_name).await?;
...
use terminusdb_relation::{DefaultField, ReverseRelation};

// Implement ReverseRelation for TdbLazy-based models
// This tells the ORM that BlogPost has a field pointing to Writer
impl ReverseRelation<Writer, DefaultField> for BlogPost {}

// Field marker for the specific 'writer' field
pub struct WriterField;
impl terminusdb_relation::RelationField for WriterField {
    fn field_name() -> &'static str {
        "writer"
    }
}
impl ReverseRelation<Writer, WriterField> for BlogPost {}
...
#[tokio::test]
async fn test_orm_with_relation_loads_children() -> anyhow::Result<()> {
    let server = TerminusDBServer::test_instance().await?;
    let client = server.client().await?;

    let db_name = "orm_with_relation_test";
    let _ = client.delete_database(db_name).await;
    client.ensure_database(db_name).await?;
