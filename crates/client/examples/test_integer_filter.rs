...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result: Vec<TestItem> = client
    ...
    if let Some(last_query) = client.last_query() {
        ...
        println!(
            "{}",
            serde_json::to_string_pretty(&last_query)
    ...
    if result.len() == 2 {
        ...
        println!(
            "\n❌ Integer filtering failed - expected 2 items, got {}",
            result.len()
        );
    }
    ...
    println!(
        "\nDatabase '{}' left for inspection. Clean up manually.",
        test_db
    );
