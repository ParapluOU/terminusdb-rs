...
fn main() {
    ...
    match serde_json::from_value::<ApiResponse<serde_json::Value>>(error_json.clone()) {
        Ok(ApiResponse::Error(typed_err)) => {
            ...
            match typed_err {
                TypedErrorResponse::InsertDocumentError {
                    error: err_response,
                    ..
                } => {
                    println!("Error message: {}", err_response.api_message);

                    if let Some(ApiResponseError::InsertedSubdocumentAsDocument(subdoc_err)) =
                        err_response.api_error
                    {
                        ...
                        println!(
                            "- Make sure the model is not marked with #[tdb(subdocument = true)]"
                        );
