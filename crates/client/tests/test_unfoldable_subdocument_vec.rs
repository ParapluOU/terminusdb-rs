...
async fn test_unfoldable_with_subdocument_vec() -> Result<()> {
    ...
    server
        .with_tmp_db("test_unfoldable_subdoc", |client, spec| async move {
            ...
            client
                .insert_entity_schema::<IdAndTitle>(args.clone())
                .await?;
            client
                .insert_entity_schema::<ReviewSession>(args.clone())
                .await?;
            ...
            let insert_result = client
                .insert_instance(&review_session, args.clone())
                .await?;
            ...
            println!(
                "Retrieved {} sessions without unfolding",
                sessions_no_unfold.len()
            );
            assert_eq!(
                sessions_no_unfold.len(),
                1,
                "Should retrieve exactly one session"
            );

            // Check what the subdocuments look like without unfolding
            println!(
                "Without unfolding - Number of document_ids: {}",
                sessions_no_unfold[0].document_ids.len()
            );
            ...
            println!(
                "Number of document_ids: {}",
                first_session.document_ids.len()
            );

            assert_eq!(
                first_session.document_ids.len(),
                3,
                "Should have 3 subdocuments"
            );
            assert_eq!(first_session.document_ids[0].id, "doc1");
            assert_eq!(
                first_session.document_ids[0].title,
                Some("Document One".to_string())
            );
            ...
            assert_eq!(
                first_session.document_ids[2].title,
                Some("Document Three".to_string())
            );
...
async fn test_empty_subdocument_vec() -> Result<()> {
    ...
    server
        .with_tmp_db("test_empty_subdoc", |client, spec| async move {
            ...
            client.insert_instance(&empty_session, args.clone()).await?;
