...
mod tests {
    ...
    #[tokio::test]
    async fn test_last_query_initial_state() -> anyhow::Result<()> {
    ...
    #[tokio::test]
    #[ignore = "query_string DSL parsing and storage not fully implemented - see test_last_query_via_operation_log"]
    async fn test_query_string_dsl_storage() -> anyhow::Result<()> {
        let server = TerminusDBServer::test_instance().await?;
        let client = server.client().await?;
        ...
        let _result = client
            .query_string::<serde_json::Value>(None, dsl_query, None)
            .await;
    ...
    #[tokio::test]
    #[ignore = "query_string DSL parsing and storage not fully implemented - see test_last_query_via_operation_log"]
    async fn test_last_query_clone_sharing() -> anyhow::Result<()> {
        let server = TerminusDBServer::test_instance().await?;
        let client = server.client().await?;
