...
impl std::cmp::Ord for ContactMethod {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match (self, other) {
            ...
            (ContactMethod::Phone { .. }, ContactMethod::Address { .. }) => {
                std::cmp::Ordering::Less
            }
            (ContactMethod::Phone { .. }, ContactMethod::SocialMedia { .. }) => {
                std::cmp::Ordering::Less
            }
            (ContactMethod::Address { .. }, ContactMethod::Email { .. }) => {
                std::cmp::Ordering::Greater
            }
            (ContactMethod::Address { .. }, ContactMethod::Phone { .. }) => {
                std::cmp::Ordering::Greater
            }
            (ContactMethod::Address { .. }, ContactMethod::SocialMedia { .. }) => {
                std::cmp::Ordering::Less
            }
...
async fn test_vec_tagged_union_subdocument_insert_and_retrieve() -> Result<()> {
    ...
    server
        .with_tmp_db("test_vec_subdoc", |client, spec| async move {
            // Insert schemas
            ...
            println!(
                "Inserted instance with commit ID: {:?}",
            ...
            println!("\n=== Raw JSON retrieval ===");
            let raw_json = client
                .get_document(&doc_id, &spec, GetOpts::default())
                .await?;
            ...
            match client
                .get_instances::<Person>(vec![short_id.clone()], &spec, opts, &mut deserializer)
                .await
            {
                Ok(persons) => {
            ...
            let raw_json_unfolded = client
                .get_document(&doc_id, &spec, GetOpts::default().with_unfold(true))
                .await?;
            println!(
                "Raw JSON with unfold: {}",
                serde_json::to_string_pretty(&raw_json_unfolded)?
            );
            ...
            assert_eq!(
                retrieved_person.notes,
                Some("Prefers email communication".to_string())
            );

            // Verify all contact methods were retrieved correctly
            assert_eq!(
                retrieved_person.contact_methods.len(),
                4,
                "Should have 4 contact methods"
            );
            ...
            match &retrieved_person.contact_methods[3] {
                ContactMethod::SocialMedia {
                    ...
                    assert_eq!(
                        url,
                        &Some("https://linkedin.com/in/alicejohnson".to_string())
                    );
                }
...
async fn test_empty_vec_tagged_union_subdocument() -> Result<()> {
    ...
    server
        .with_tmp_db("test_empty_vec", |client, spec| async move {
            // Insert schemas
            let args = DocumentInsertArgs::from(spec.clone());
            client.insert_entity_schema::<Person>(args.clone()).await?;
            ...
            let result = client.insert_instance(&person_empty, args.clone()).await?;
...
async fn test_single_variant_vec() -> Result<()> {
    ...
    server
        .with_tmp_db("test_single_variant", |client, spec| async move {
            // Insert schemas
            let args = DocumentInsertArgs::from(spec.clone());
            client.insert_entity_schema::<Person>(args.clone()).await?;
...
async fn test_hashset_tagged_union_subdocument() -> Result<()> {
    ...
    server
        .with_tmp_db("test_hashset_subdoc", |client, spec| async move {
            // Insert schemas
            let args = DocumentInsertArgs::from(spec.clone());
            client
                .insert_entity_schema::<PersonWithHashSet>(args.clone())
                .await?;
            ...
            let insert_result = client
                .insert_instance(&person_hashset, args.clone())
                .await?;
            println!(
                "Inserted HashSet instance with commit ID: {:?}",
                insert_result.commit_id
            );
            ...
            let mut deserializer = DefaultTDBDeserializer;
            match client
                .get_instances_unfolded::<PersonWithHashSet>(
                    vec![short_id],
                    &spec,
                    &mut deserializer,
                )
                .await
            {
                Ok(persons) => {
                    println!(
                        "\nSuccessfully retrieved {} PersonWithHashSet instances",
                        persons.len()
                    );
...
async fn test_btreeset_tagged_union_subdocument() -> Result<()> {
    ...
    server
        .with_tmp_db("test_btreeset_subdoc", |client, spec| async move {
            // Insert schemas
            let args = DocumentInsertArgs::from(spec.clone());
            client
                .insert_entity_schema::<PersonWithBTreeSet>(args.clone())
                .await?;
            ...
            // Insert the instance
            let insert_result = client
                .insert_instance(&person_btreeset, args.clone())
                .await?;
            println!(
                "Inserted BTreeSet instance with commit ID: {:?}",
                insert_result.commit_id
            );
            ...
            if let Some(contact_methods) =
                raw_json.get("contact_methods").and_then(|v| v.as_array())
            {
                if !contact_methods.is_empty() {
                    let first_contact = &contact_methods[0];
                    if first_contact.is_string() {
                        println!("\nBTreeSet stores subdocuments as references (not embedded)");
                    } else if first_contact.is_object() {
                        println!("\nBTreeSet stores subdocuments as EMBEDDED objects");
                        println!(
                            "First contact embedded: {}",
                            serde_json::to_string_pretty(first_contact)?
                        );
                    }
                }
            }
            ...
            let mut deserializer = DefaultTDBDeserializer;
            match client
                .get_instances_unfolded::<PersonWithBTreeSet>(
                    vec![short_id],
                    &spec,
                    &mut deserializer,
                )
