...
fn test_code_reduction_metrics() {
    ...
    println!(
        "  Before refactoring: ~{} explicit patterns",
        patterns_before
    );
    println!(
        "  After refactoring: ~{} patterns + 2 helpers",
        patterns_after
    );
    println!(
        "  Reduction: ~{}%",
        (1.0 - (patterns_after as f64 / patterns_before as f64)) * 100.0
    );
...
macro_rules! example_with_helpers {
    ...
    ($node_pattern:tt $dir:tt $next_pattern:tt) => {{
        ...
        println!(
            "Parsed: node with field {:?}, direction {:?}, next node with field {:?}",
