...
use terminusdb_schema::{Key, ServerIDFor, ToTDBInstance, ToTDBSchema};
...
fn test_valid_option_with_lexical_key() {
    assert_eq!(
        ValidLexicalWithOption::id(),
        Some("ValidLexicalWithOption".to_string())
    );
...
fn test_valid_string_with_random_key() {
    assert_eq!(
        ValidRandomWithString::id(),
        Some("ValidRandomWithString".to_string())
    );
    ...
    assert_eq!(
        tdb_instance.id,
        Some("ValidRandomWithString/custom-id".to_string())
    );
}
