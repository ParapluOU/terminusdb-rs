...
fn generate_struct_filter(
    ...
    let doc = format!(
        "GraphQL filter for `{model}`, generated by `#[derive(TerminusDBModel)]`. \
         One optional field per model field: scalar fields take a scalar filter \
         (`StringFilter`/`IntFilter`/…), relation fields take the target's boxed \
         `{{Target}}Filter` (nested), enums take `{{ eq, ne }}` — plus the \
         `tdb_id`/`tdb_ids`/`tdb_and`/`tdb_or`/`tdb_not` envelope (serde-renamed to \
         `_id`/`_ids`/`_and`/`_or`/`_not`). Build it and pass to `Model::query(..)` or \
         `RelationOpts::filter(..)`; it renders to GraphQL via the `ToGql` impl below. \
         See the terminusdb-orm README (\"Filtering\")."
    );
...
fn generate_enum_filter(
    ...
    let doc = format!(
        "GraphQL enum filter for `{model}` ({{ eq, ne }}), generated by \
         `#[derive(TerminusDBModel)]`. Enum values render bare via `ToGql`."
    );
