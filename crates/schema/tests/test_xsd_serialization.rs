...
fn test_xsd_any_simple_type_serialization_fixed() {
    ...
    use terminusdb_schema::{InstanceProperty, PrimitiveValue, Schema, ToInstanceProperty};
    ...
    let decimal_prop: InstanceProperty =
        <XSDAnySimpleType as ToInstanceProperty<()>>::to_property(decimal_value, "test", &schema);
    ...
    let uint_prop: InstanceProperty =
        <XSDAnySimpleType as ToInstanceProperty<()>>::to_property(uint_value, "test", &schema);

    match uint_prop {
        InstanceProperty::Primitive(PrimitiveValue::Object(json)) => {
            println!(
                "UnsignedInt serialization: {}",
                serde_json::to_string_pretty(&json).unwrap()
            );
