...
fn test_concat_generates_correct_json_ld() {
    ...
    assert_eq!(
        first_elem.get("@type").unwrap().as_str().unwrap(),
        "DataValue"
    );
    assert_eq!(
        first_elem.get("data").unwrap().as_str().unwrap(),
        "AwsDBPublication/"
    );
    ...
    assert_eq!(
        second_elem.get("@type").unwrap().as_str().unwrap(),
        "DataValue"
    );
    assert_eq!(
        second_elem.get("variable").unwrap().as_str().unwrap(),
        "PubId"
    );
