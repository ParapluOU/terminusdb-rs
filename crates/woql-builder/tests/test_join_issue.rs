...
fn test_join_generates_correct_json_ld() {
    ...
    let query = WoqlBuilder::new()
        .join(
            list(vec![string_literal("Hello"), string_literal("World")]),
            string_literal(", "),
            result_var.clone(),
        )
        .finalize();
    ...
    assert!(
        list_value.is_array(),
        "list should be a direct array, not wrapped in DataValue"
    );
    ...
    assert_eq!(
        separator_obj.get("@type").unwrap().as_str().unwrap(),
        "DataValue"
    );
...
fn test_join_with_variable_list() {
    ...
    let query = WoqlBuilder::new()
        .join(parts_var.clone(), string_literal(" - "), result_var.clone())
        .finalize();
    ...
    assert_eq!(
        list_obj.get("@type").unwrap().as_str().unwrap(),
        "DataValue"
    );
